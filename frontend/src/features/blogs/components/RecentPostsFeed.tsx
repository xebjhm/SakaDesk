// frontend/src/features/blogs/components/RecentPostsFeed.tsx
// Portrait Gallery Feed - Multi-group themed with dynamic ambient background
import React, { useMemo } from 'react';
import type { RecentPost } from '../../../types';
import { BlogCard } from './BlogCard';
import { DynamicBackground } from '../../../core/common/DynamicBackground';
import { useAppStore } from '../../../store/appStore';
import type { BlogSelectionMode } from '../../../store/appStore';
    ...
    return (
        <div className="feed-container" style={{ background: theme.surface.background }}>
            ...
            <header className="feed-header">
                <div className="feed-header__inner">
                    <div className="feed-header__titles">
                        <h2 className="feed-header__main">{t('blogs.latestBlogs')}</h2>
                        {syncing && (
                            <span className="feed-header__sync-indicator">
                                <svg className="animate-spin w-3 h-3" fill="none" viewBox="0 0 24 24">
                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                                </svg>
                                <span>{t('blogs.checkingUpdates')}</span>
                            </span>
                        )}
                    </div>
            ...
            <main className="feed-content">
                ...
                {loading && (
                    <div className="feed-state">
                        ...
                        <span className="feed-state__text">{t('blogs.loadingPosts')}</span>
                    </div>
                )}
                ...
                {!loading && !error && posts.length === 0 && !syncing && (
                    <div className="feed-state">
                        ...
                        <p className="feed-state__text">
                            {selectionMode === 'favorite'
                                ? t('blogs.noPostsFromFavorites')
                                : t('blogs.noBlogPostsFound')}
                        </p>
                        <p className="feed-state__hint">
                            {selectionMode === 'favorite'
                                ? t('blogs.addMembersHint')
                                : t('blogs.selectServiceHint')}
                        </p>
                    </div>
                )}
            </main>
            ...
            <style>{`
                ...
                .feed-header__sync-indicator svg {
