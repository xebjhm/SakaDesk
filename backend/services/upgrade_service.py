...
async def download_installer(
    ...
    """
    # Reject non-HTTPS / non-GitHub URLs before touching the network.
    if not _is_allowed_download_url(info.url):
        logger.error(
            "Refusing installer download from disallowed URL",
            url=info.url,
        )
        return None
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=300.0, follow_redirects=True) as client:
            async with client.stream("GET", info.url) as response:
                ...
                # Re-validate the URL we actually landed on: redirects are
                # followed, so a redirect off GitHub must be rejected before we
                # read any bytes.
