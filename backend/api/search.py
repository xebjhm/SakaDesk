...
async def search_messages(
    ...
    service: str = Query(
        None, description="Filter by service (single, for nickname resolution)"
    ),
    group_id: int = Query(None, description="Filter by talk room ID"),
    member_id: int = Query(None, description="Filter by member ID"),
    services: str = Query(
        None, description="Filter by services (comma-separated, OR logic)"
    ),
    member_ids: str = Query(
        None, description="Filter by member IDs (comma-separated, OR logic, legacy)"
    ),
    member_filters: str = Query(
        None,
        description="Filter by service:member_id pairs (comma-separated, OR logic)",
    ),
    exact_only: bool = Query(
        False, description="Exact match only - skip pronunciation/reading matching"
    ),
    exclude_unread: bool = Query(
        False, description="Exclude unread messages (requires read_states)"
    ),
    ...
    content_type: str = Query(
        "all", description="Content type filter: all, messages, blogs"
    ),
    ...
    if member_ids:
        try:
            member_ids_list = [
                int(m.strip()) for m in member_ids.split(",") if m.strip()
            ]
        except ValueError:
            raise HTTPException(
                status_code=400, detail="Invalid member_id: must be integers"
            )
