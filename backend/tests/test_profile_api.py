...
class TestGetProfile:
    ...
    def test_returns_cached_nickname(self):
        ...
        with patch("backend.api.profile._store_load", new_callable=AsyncMock) as mock_load:
        ...
        with patch("backend.api.profile._store_load", new_callable=AsyncMock) as mock_load:
            ...
            with patch("backend.api.profile.get_token_manager") as mock_tm:
                ...
                with patch("backend.api.profile.Client") as mock_client_cls:
                    ...
                    mock_client.get_profile = AsyncMock(return_value={"name": "FreshNick"})
                    mock_client_cls.return_value = mock_client

                    with patch("backend.api.profile.aiohttp.ClientSession") as mock_session_cls:
                        mock_session = AsyncMock()
                        mock_session_cls.return_value.__aenter__ = AsyncMock(return_value=mock_session)
                        mock_session_cls.return_value.__aexit__ = AsyncMock(return_value=False)

                        with patch("backend.api.profile._store_update", new_callable=AsyncMock) as mock_update:
                            response = client.get("/api/profile?service=hinatazaka46")
    ...
    def test_api_returns_no_name(self):
        ...
        with patch("backend.api.profile._store_load", new_callable=AsyncMock) as mock_load:
            ...
            with patch("backend.api.profile.get_token_manager") as mock_tm:
                ...
                with patch("backend.api.profile.Client") as mock_client_cls:
                    mock_client = MagicMock()
                    mock_client.get_profile = AsyncMock(return_value={"email": "x@y.com"})
                    mock_client_cls.return_value = mock_client
    ...
    def test_api_exception_returns_error(self):
        ...
        with patch("backend.api.profile._store_load", new_callable=AsyncMock) as mock_load:
            mock_load.return_value = {"user_nicknames": {}}
            with patch("backend.api.profile.get_token_manager") as mock_tm:
                mock_tm.return_value.load_session.side_effect = RuntimeError("keyring broken")
                response = client.get("/api/profile?service=hinatazaka46")
        ...
        with patch("backend.api.profile._store_load", new_callable=AsyncMock) as mock_load:
            ...
            with patch("backend.api.profile._store_update", new_callable=AsyncMock) as mock_update:
                with patch("backend.api.profile.get_token_manager") as mock_tm:
                    ...
                    with patch("backend.api.profile.Client") as mock_client_cls:
                        mock_client = MagicMock()
                        mock_client.get_profile = AsyncMock(return_value={"name": "NewNick"})
                        mock_client_cls.return_value = mock_client
    ...
    def test_refresh_clears_only_target_service(self):
        ...
        with patch("backend.api.profile._store_update", side_effect=capture_update):
            with patch("backend.api.profile._store_load", new_callable=AsyncMock) as mock_load:
                ...
                mock_load.return_value = {"user_nicknames": {}}
                with patch("backend.api.profile.get_token_manager") as mock_tm:
                    mock_tm.return_value.load_session.return_value = {"access_token": "t"}
                    with patch("backend.api.profile.Client") as mock_client_cls:
                        mock_client = MagicMock()
                        mock_client.get_profile = AsyncMock(return_value={"name": "New"})
                        mock_client_cls.return_value = mock_client
