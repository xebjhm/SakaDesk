...
| `onnxruntime` + embedding model | **SakaDesk backend only** | MIT/Apache | native+heavy; never a pysaka dep (would bloat all PyPI consumers, risks py3.9) |
...
1. compute a **cross-lingual embedding** via the **injected `Embedder`** (SakaDesk `onnxruntime` impl). **Default model: `multilingual-e5-base`** (stronger cross-lingual recall than `-small`, ~140 MB int8 ONNX), applying the **required `query:`/`passage:` prefixes** + NFKC; swappable to `bge-m3` for higher quality (D2). Offloaded to an executor (async hygiene).
...
Embedding model `multilingual-e5-base` (int8 ONNX, `query:`/`passage:` prefixes) · vector = numpy flat cosine · RRF `k=60`, equal weight · blog chunk 400 tok / 15% overlap · message neighbor window ±1 · max tool-steps 6 (+1 synth +1 repair) · `content_hash` over cleaned text · fuzzy threshold token-set ratio ≥0.9 · canonical key = synthetic.

## 16. Open items requiring your input
1. **Text-first vs. the food/outing examples (the one real tension).** Dish/location facts often live only in photos/video, which v1 excludes — so those two headline questions are answered *only from text/captions* and will caveat partial-ness. **Confirm** text-first v1 is acceptable, **or** approve pulling **on-demand vision** (your earlier option B: vision model on candidate images at query time, user's key) into v1 for the food/outing class.
2. **Alias source-of-truth file** — you'll provide it (D4); confirm the §6.4 schema or hand me your format and I adapt the loader. (Seed aliases work meanwhile.)
3. **Embedding model footprint** — `e5-base` (~140 MB int8) default; OK to ship as a pinned GitHub Release asset, or prefer `-small` (smaller, weaker) / `bge-m3` (bigger, stronger)?

## 17. Review disposition (what changed)
**Corrected facts:** blog FTS5 already exists & sync-wired (Phase-1 scope reduced); 2 keyspaces not 3; `search_index.db`; bs4 already present (no GPL `html2text`). **Design fixes:** embedder inverted out of pysaka behind a protocol + dependency-placement matrix; `scope` typed with a default; streaming resolved as two-pass vs. the validator; canonical key = synthetic (§6.4/§6.1 aligned); conversation working-set server-side with cross-turn provenance; validator uses structured per-sentence citations + pinned normalization/threshold; caption-less media = metadata-only docs; new-member auto-provision; on-demand-transcript re-index trigger; timezone via request `tz`; lexical-only EN/ZH degradation acknowledged; cost restated (~3–6 calls/q); embedding upgraded to `e5-base` + prefixes; `aggregate` promoted to v1; numpy flat vector (no sqlite-vec/hnswlib); golden eval = checked-in synthetic fixtures + numeric gate; Anthropic dropped; concrete defaults in §15. **Reframe:** product promise = grounded best-effort with explicit partial-ness (D7), with §7.6 documenting recall ceilings.
